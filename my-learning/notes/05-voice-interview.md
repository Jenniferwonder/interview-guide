...
- **WebSocket vs WebRTC**：WebSocket 适合信令和数据，WebRTC 适合音视频直连（更低延迟）
- **为什么不用端到端语音模型**：当前链条（ASR→LLM→TTS）灵活可替换，端到端是演进方向
- **VAD 为什么在服务端**：不依赖客户端浏览器兼容性
- **回声防护**：AI 说话时暂停收音，这是最简单的回声消除策略
